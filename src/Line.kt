...
open class Line(val a: Double, val b: Double, val c: Double) {
    ...
    open fun intersections(circle: Circle): List<Intersection> {
        ...
        val line = translate(circle.center.scale(-1.0))
        val a = line.a
        val b = line.b
        val c = line.c
        ...
        if (d0 == r) { // the point is exactly on the circle
            val p0 = Intersection(Point(x0, y0).add(circle.center), this)
            return listOf(p0)
        }
        ...
        val x1 = x0 + b * m
        val y1 = y0 - a * m
        val p1 = Point(x1, y1).add(circle.center)
        val i1 = Intersection(p1, this)

        val x2 = x0 - b * m
        val y2 = y0 + a * m
        val p2 = Point(x2, y2).add(circle.center)
    ...
    fun equals2(other: Line): Boolean {
        val selfX = -c / a
        val selfY = -c / b
